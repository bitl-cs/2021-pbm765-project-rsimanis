...
void render_lobby(){
    ...
    for (i = 0; i < player_count; i++) {
        ...
        if (rend_data.client_ids_in_lobby[i] != client_id) {
            rend_data.client_ids_in_lobby[i] = client_id;
            strcpy(rend_data.client_names_in_lobby[i], name);
        }
        if(rend_data.client_id != rend_data.client_ids_in_lobby[i]){
...
void lobby_button_listener(int button, int event, int x, int y){
    ...
    int player_id_index = match_id_to_index(rend_data.client_id);
    ...
    if(button == GLUT_LEFT_BUTTON && event == GLUT_DOWN){
        ...
        if(button_pressed(CHAT_SEND_ALL_BUTTON_X, CHAT_SEND_ALL_BUTTON_Y, CHAT_SEND_ALL_BUTTON_WIDTH, CHAT_SEND_ALL_BUTTON_HEIGHT, x, y)){
            ...
            send_message_from_client(PACKET_MESSAGE_TARGET_ALL, rend_data.client_id, inp_data.input_buf, rend_data.send_mem);
        ...
        for (i = 0; i < 4; i++) {
            if(i != player_id_index && button_pressed(CHAT_SEND_BUTTON_X, CHAT_SEND_BUTTON_INITIAL_Y + i * (CHAT_SEND_BUTTON_PADDING + CHAT_SEND_BUTTON_HEIGHT) , 
                ...
                send_message_from_client(rend_data.client_ids_in_lobby[i], rend_data.client_id, inp_data.input_buf, rend_data.send_mem);
                clear_input_buffer();
            }
        }
    }
